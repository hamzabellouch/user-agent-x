...
- **Experimental Capsule Design:** Fully rounded capsule-shaped buttons (`border-radius: 50px`) for a more premium and fluid user experience.
- **Preconfigured Profiles:** Switch between popular browsers (Chrome, Firefox, Safari, Edge, Opera, Brave) and platforms (Windows, macOS, Linux, Android, iOS).
- **Custom User-Agent Support:** Manually input and apply any custom User-Agent string.
- **Client Hint Spoofing & Blocking:** Automatically sets matching `sec-ch-ua` headers for Chromium profiles, or strips them entirely for non-Chromium profiles (Firefox/Safari).
...
│   ├── manifest.json  # Chromium Manifest V3 configuration (v1.5.0)
│   ├── popup.css      # Capsule UI styling
...
│   ├── content.js     # Client Hints override injection script
│   ├── manifest.json  # Firefox Manifest V3 configuration (v1.5.0, Gecko ID)
│   ├── popup.css      # Capsule UI styling
│   ├── popup.js       # UI interaction logic
│   └── popup.html     # Popup user interface
│
...
> [!WARNING]
> This is a beta/experimental branch. There is always a possibility of website breakage or detection when using advanced spoofing, so we assume no responsibility for any inaccuracies or issues.
